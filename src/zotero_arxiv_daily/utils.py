...
def extract_tex_code_from_tar(file_path: str, paper_id: str, paper_title: str | None = None) -> dict[str, str]:
    try:
        with tarfile.open(file_path) as tar:
            return _extract_from_open_tar(tar, file_path, paper_id, paper_title)
    except tarfile.ReadError:
        logger.debug(f"Failed to find main tex file of {paper_id}: Not a tar file.")
        return None


def _extract_from_open_tar(
    tar: tarfile.TarFile, file_path: str, paper_id: str, paper_title: str | None
) -> dict[str, str]:
